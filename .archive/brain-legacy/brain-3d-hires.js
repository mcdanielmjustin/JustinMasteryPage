...
function toggleSplit() {
  ...
  if (_splitOn) {
    ...
    // DoubleSide so the cut-edge interior renders — prevents the hollow black gap
    // that appears at the midplane seam when only FrontSide faces are drawn.
    if (brainGroup) {
      brainGroup.traverse(function(child) {
        if (!child.isMesh || !child.material) return;
        child.material.side = THREE.DoubleSide;
    ...
    if (brainGroup) {
      brainGroup.traverse(function(child) {
        if (!child.isMesh || !child.material) return;
        // Restore: keep DoubleSide if glass is also on, otherwise back to FrontSide
